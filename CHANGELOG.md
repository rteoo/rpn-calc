...
- **The app is called "RPN Calc".** The window title, Start menu and desktop
  shortcuts, uninstall entry, Windows file properties, and macOS/iOS app
  names read "RPN Calc" instead of "rpn-calc". Upgrading through the Windows
  installer removes the old `rpn-calc` Start menu folder and desktop
  shortcut. File names, taskbar pins, and saved settings are unchanged.

## [0.6.3] - 2026-09-22

### Changed

- **Faster startup.** The window opens about 40% sooner (~755 ms to ~458 ms
  on the Windows build). Hidden keycap text no longer carries the drawn Σ
  and Δ, which sent Qt searching the system fonts for glyphs nobody sees.
  The rendered face is unchanged.
...
- **README improvements.** The product overview and usage guidance have been rewritten for clarity, making it easier for new users to understand the calculator's capabilities and get started.
- **Release infrastructure.** Build credentials are now scoped and build inputs locked to improve security and reproducibility in CI/CD pipelines.
