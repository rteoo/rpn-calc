...
class RpnStack:
    def __init__(self) -> None:
        ...
        # The arguments consumed by the last mutating command, for UNDO
        # (`restore_last_args`). Recorded as a full stack snapshot taken
        # before the mutation, since HP's UNDO restores the whole stack to
        # its pre-command state rather than re-pushing individual values.
        self.last_args: list[float] | None = None
    ...
    def _snapshot(self) -> None:
        self.last_args = list(self._levels)
    ...
    def restore_last_args(self) -> None:
        """UNDO: put the stack back to its state before the last mutation."""
        if self.last_args is not None:
            self._levels = list(self.last_args)
